...
import {
  serializePathIdentifierAndMile,
  pathIdentifierToName,
  getOgImageUrl,
  getMilePath,
  geocodeToLocationString,
} from "../../../utils";
...
const Mile = ({ section, geocode, maxElevation, diffElevation }: MileProps) => {
    ...
    adjacentMiles.push(
      <Link href={getMilePath(path, previousMile)}>
        <button className="rounded-full">
          &larr; Previous Mile ({previousMile})
        </button>
      </Link>
    );
  }
  ...
  const elevGainFeetDisplay = parseInt(
    metersToFeet(diffElevation).toFixed(),
    10
  ).toLocaleString();
  const elevGainMetersDisplay = parseInt(
    diffElevation.toFixed()
  ).toLocaleString();

  const maxElevFeetDisplay = parseInt(
    metersToFeet(maxElevation).toFixed(0)
  ).toLocaleString();
      ...
      <section className="">
        <div className="px-4 mx-auto text-center max-w-7xl sm:px-6 lg:px-8">
          ...
          <p className="mt-5 md:w-1/2 mx-auto">
            ...
            elevation change of{" "}
            <strong>
              {elevGainFeetDisplay} ft ({elevGainMetersDisplay} m)
            </strong>{" "}
            with a maximum elevation of{" "}
            <strong>
              {maxElevFeetDisplay} ft ({maxElevMetersDisplay} m)
            </strong>
            .
          </p>
          <div
          ...
          >
            <MapboxMap
              ...
              onLoaded={(map) => {
                setMap(map);
                map.loadImage(
                  "https://every-mile.vercel.app/images/chevron.png",
                  (error, image) => {
                    if (error || !image) {
                      return;
                    }
...
export const getStaticPaths: GetStaticPaths = async () => {
  return new Promise((resolve) => {
    ...
    const atPaths = Array(DISTANCES.at)
      .fill(1)
      .map((_, index) => ({
        params: {
          path_slug: "appalachian-trail",
          mile: (index + 1).toString(),
        },
      }));
    const brpPaths = Array(DISTANCES.brp)
      .fill(1)
      .map((_, index) => ({
        params: {
          path_slug: "blue-ridge-parkway",
          mile: (index + 1).toString(),
        },
      }));
